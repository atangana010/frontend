...
export default function VerificationMatricule() {
  ...
  const {
    chargement, erreur, etudiant,
    rechercherParMatricule, passerAuPaiement, reinitialiserRecherche,
  } = useVerification();
  ...
  const handleRechercher = async (e) => {
    ...
    if (mat.length < 3) {
      setErreurLocale('Matricule trop court (minimum 3 caractères)');
      // Tracker la tentative invalide
      track('matricule_invalide', { longueur: mat.length });
      return;
    }

    // Tracker la recherche
    track('matricule_recherche');

    const succes = await rechercherParMatricule(mat);

    if (succes) {
      // Tracker le succès de la vérification
      track('matricule_verifie', { filiere: etudiant?.filiere || 'inconnue' });
    } else {
      // Tracker l'échec
      track('matricule_introuvable');
    }
  };

  const handleSuivant = () => {
    // Tracker le passage à l'étape paiement
    track('passer_au_paiement', { filiere: etudiant?.filiere || 'inconnue' });
    passerAuPaiement();
  };
  ...
  const handleReinitialiser = () => {
    setMatricule('');
    setErreurLocale(null);
    track('reinitialiser_verification');
        ...
        <>
          ...
          <div className="carte p-8">
            <form onSubmit={handleRechercher} className="space-y-5">
              ...
              {erreurLocale && (
                <Notification type="erreur" message={erreurLocale}
                  onFermer={() => setErreurLocale(null)} />
              )}
              {erreur && !erreurLocale && (
                <Notification type="erreur" message={erreur} />
              )}
            ...
            <div className="mt-6 pt-6 border-t border-gray-100 flex items-center justify-between">
              <span className="text-sm text-gray-500">Frais d'inscription</span>
              <span className="font-bold font-mono text-base" style={{ color: '#227838' }}>
                100 000 FCFA
              </span>
            </div>
          </div>
      ...
      {etudiant && (
        <div className="animate-in space-y-4">
          <div className="flex items-center gap-3 p-4 rounded-xl"
            style={{ backgroundColor: '#dcfce7', border: '1px solid #86efac' }}>
            <svg className="w-5 h-5 flex-shrink-0" style={{ color: '#166534' }}
            ...
            <div className="flex items-center gap-4 mb-6 pb-6 border-b border-gray-100">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  <span className="font-mono font-semibold" style={{ color: '#227838' }}>
                    {etudiant.matricule}
                  </span>
          ...
          <div className="carte p-4 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-700">Frais d'inscription 2025–2026</p>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={handleSuivant} className="btn-primaire flex-1 text-base py-4">
              Suivant
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
